...
$(document).ready(function(){
    ...
    $('.panel-inactive').on('click', function(){
        ...
        $('.panel-active').children(".panel-heading").children(".panel-title-logo").show();
    ...
    // on load
        $("input:checkbox[name='layer-control']").each(function(){
            ...
            input = $(this).parents('.layer-box').siblings('.title').text().toLowerCase();
            var id = $(this).attr('id');
    ...
    $("input:checkbox[name='layer-control']").on('change', function(){
        ...
        // get the affected pane
        input = $(this).parents('.layer-box').siblings('.panel-title').children('.title').text().toLowerCase();
        console.log(input);
        var id = $(this).attr('id');
        // check to see if a checkbox is checked and if so, push it to a global array, and if not, remove it from map
        if ($('#'+id).prop('checked')){
