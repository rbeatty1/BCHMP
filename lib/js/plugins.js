                    ...
                    if (allLayers[z].layers[y].name === visible[x].layer){
                        ...
                        if (allLayers[z].layers[y].qry != undefined){
                            lyr = L.esri.featureLayer({
                                ...
                                console.log(lyr, lyrName);
                                md.push({
                                    name: this,
                                    style: metadata.drawingInfo.renderer
                                });
                            });
...
function updateLegend(visibleLayers, metadata){
    for (var key in metadata){
        var metaLength = metadata.length;
        console.log(metaLength);
        var metaName = metadata[key].name;
        for (i = 0; visibleLayers.length; i++){
            var vizName = visibleLayers[i].layer;
            if (vizName != metaName){
                console.log(vizName+" "+metaName+" no match");
            }
            else{
                $(document).ready(function(){$("#legend-"+metaName).show();});
            }
        }
    }
}